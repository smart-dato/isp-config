<?php

declare(strict_types=1);

namespace SmartDato\IspConfig\Resources\Mail;

use SmartDato\IspConfig\Resources\Resource;

final class User extends Resource
{
    /**
     * @param  array<string, mixed>  $params
     */
    public function add(int $clientId, array $params): int
    {
        /** @var int */
        return $this->call('mail_user_add', $clientId, $params);
    }

    /**
     * @return array<string, mixed>
     */
    public function get(int $id): array
    {
        /** @var array<string, mixed> */
        return $this->call('mail_user_get', $id);
    }

    /**
     * @return list<array<string, mixed>>
     */
    public function list(int $domainId): array
    {
        /** @var list<array<string, mixed>> */
        return $this->call('mail_user_list', $domainId);
    }

    /**
     * @param  array<string, mixed>  $params
     */
    public function update(int $clientId, int $id, array $params): int
    {
        /** @var int */
        return $this->call('mail_user_update', $clientId, $id, $params);
    }

    public function delete(int $id): int
    {
        /** @var int */
        return $this->call('mail_user_delete', $id);
    }

    public function instantBackup(int $id): mixed
    {
        return $this->call('mail_user_instant_backup', $id);
    }

    /**
     * @param  array<string, mixed>  $params
     */
    public function backup(int $id, array $params): mixed
    {
        return $this->call('mail_user_backup', $id, $params);
    }

    /**
     * @return list<array<string, mixed>>
     */
    public function backupList(int $id): array
    {
        /** @var list<array<string, mixed>> */
        return $this->call('mail_user_backup_list', $id);
    }
}
