<?php

declare(strict_types=1);

namespace SmartDato\IspConfig\Resources\Mail;

use SmartDato\IspConfig\Resources\Resource;

final class Domain extends Resource
{
    /**
     * @param  array<string, mixed>  $params
     */
    public function add(int $clientId, array $params): int
    {
        /** @var int */
        return $this->call('mail_domain_add', $clientId, $params);
    }

    /**
     * @return array<string, mixed>
     */
    public function get(int $id): array
    {
        /** @var array<string, mixed> */
        return $this->call('mail_domain_get', $id);
    }

    /**
     * @return array<string, mixed>
     */
    public function getByDomain(string $domain): array
    {
        /** @var array<string, mixed> */
        return $this->call('mail_domain_get_by_domain', $domain);
    }

    /**
     * @param  array<string, mixed>  $params
     */
    public function update(int $clientId, int $id, array $params): int
    {
        /** @var int */
        return $this->call('mail_domain_update', $clientId, $id, $params);
    }

    public function delete(int $id): int
    {
        /** @var int */
        return $this->call('mail_domain_delete', $id);
    }

    public function setStatus(int $id, string $status): mixed
    {
        return $this->call('mail_domain_set_status', $id, $status);
    }
}
